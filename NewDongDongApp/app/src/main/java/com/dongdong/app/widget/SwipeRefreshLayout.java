...
public class SwipeRefreshLayout extends ViewGroup {
	...
	private final Runnable mCancel = new Runnable() {
		...
		public void run() {
			...
			if (mProgressBar != null || mProgressBarBottom != null) {
				...
				if (mDirection > 0
						&& ((mMode == Mode.PULL_FROM_START) || (mMode == Mode.BOTH))) {
					...
					startAnimation(mShrinkTrigger);
						...
						&& ((mMode == Mode.PULL_FROM_END) || (mMode == Mode.BOTH))) {
					mShrinkTriggerBottom.setDuration(mMediumAnimationDuration);
					mShrinkTriggerBottom
							.setAnimationListener(mShrinkAnimationListener);
	 ...
	 */
	@Deprecated
	private void setColorScheme(int colorRes1, int colorRes2, int colorRes3,
			int colorRes4) {
	...
	public void setBottomColor(int colorRes1, int colorRes2, int colorRes3,
			int colorRes4) {
		setColorSchemeResourcesBottom(colorRes1, colorRes2, colorRes3,
				colorRes4);
	}
	...
	private void setColorSchemeResourcesBottom(int colorRes1, int colorRes2,
			int colorRes3, int colorRes4) {
		final Resources res = getResources();
		setColorSchemeColorsBottom(res.getColor(colorRes1),
				res.getColor(colorRes2), res.getColor(colorRes3),
	 ...
	 * gesture.
	 */
	private void setColorSchemeColors(int color1, int color2, int color3,
			int color4) {
		ensureTarget();
		mProgressBar.setColorScheme(color1, color2, color3, color4);
	}

	private void setColorSchemeColorsBottom(int color1, int color2, int color3,
			int color4) {
		ensureTarget();
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		child.layout(childLeft, childTop, childLeft + childWidth, childTop
				+ childHeight);
		mProgressBarBottom.setBounds(0, height - mProgressBarHeight, width,
				height);
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if ((canChildScrollUp() && yDiff > 0)
					|| (canChildScrollDown() && yDiff < 0)) {
				mStartPoint = y;
			}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (!mIsBeingDragged
					&& (yDiff > 0 && mLastDirection == Mode.PULL_FROM_START)
			...
			if (mIsBeingDragged) {
				...
				if (yDiff > mDistanceToTriggerSync) {
					...
					setTriggerPercentage(mAccelerateInterpolator
							.getInterpolation(Math.abs(yDiff)
